...
class CSEMachine {
    ...
    private String getValueOfToken(String token) {
        // some input may have spaces. thats why trim
        token = token.trim();
    ...
    void runCSEMachine() {
        try {
            ...
            stack.push("PE_0");
            // get the last element from the control structure and examine if its jus an
            // identifier or gamma
            ...
            while (controlStructure.size() > 0) { // the last left out value will be the PE_0 env
                ...
                // Controlling the unary and binary operations.
                if (item.equals("")) {
                    continue;
                    ...
                    } else { // RULE 9 this means there is only a single tau operator. Evaluate the _ arg and
                             // see how many elements are there. pop that many values frm the stack
                    ...
                    if (itemAtTopOfStack.equals("<Y*>")) {
                        ...
                        if (nextStackTop.startsWith(STTransformer.LAMBDA)) {
                            // we should push Eta_i_v:c, Workaround is to rename lambda into eta and push it
                            // into the stack.
                    ...
                    else if (!itemAtTopOfStack.startsWith(STTransformer.LAMBDA)) { // if its not lambda, then apply
                                                                                   // rator to rand and push it back.
                        String rator = (String) stack.pop();
                        String rand = (String) stack.pop();
                        String result = apply(item, rator, rand);
                        stack.push(result);
                        ...
                        if (X.indexOf(',') >= 0) {
                            ...
                            environmentHash.put("PE_" + Counter, currEnvMapping);
                        }
    ...
    // Method for remove the Delta function
